...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (isset($_FILES['goods_front_cover']['error'])){ // php 4.2 版本才支持 error
        // 最大上传文件大小
        $php_maxsize = ini_get('upload_max_filesize');
        $htm_maxsize = '2M';
    
        // 商品图片
        if ($_FILES['goods_front_cover']['error'] == 0)
        {
            if (!$image->check_img_type($_FILES['goods_front_cover']['type']))
            {
                sys_msg($_LANG['invalid_goods_img'], 1, array(), false);
            }
        }
        elseif ($_FILES['goods_front_cover']['error'] == 1)
        {
            sys_msg(sprintf($_LANG['goods_img_too_big'], $php_maxsize), 1, array(), false);
        }
        elseif ($_FILES['goods_front_cover']['error'] == 2)
        {
    ...
    // 是否上传商品封面图
    if (isset($_FILES['goods_front_cover']) && $_FILES['goods_front_cover']['tmp_name'] != '' &&
            isset($_FILES['goods_front_cover']['tmp_name']) &&$_FILES['goods_front_cover']['tmp_name'] != 'none')
    {
        // 上传了，直接使用，原始大小
        // $dir = date('Ym');
        ...
        $fileName = substr(md5($rand_name), 8, 16) .".png";
        $dir = date('Ym');
        $dir = 'images/'.$dir.'/'.$fileName;

        $front_cover_thumb = $image->imagecropper($_FILES['goods_front_cover']['tmp_name'], 200, 267,$dir);
        $randNumber = mt_rand(00000, 99999). mt_rand(000, 999);
        $fileNamea = substr(md5($randNumber), 8, 16) .".png";
        $dira = date('Ym');
        $dira = 'images/'.$dira.'/'.$fileNamea;
        $goods_front_cover = $image->imagecropper($_FILES['goods_front_cover']['tmp_name'], 660, 340,$dira);
    }
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, goods_sex)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$_POST[goods_sex]')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, goods_sex)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', '$goods_front_cover', '$front_cover_thumb', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$_POST[goods_sex]')";
        }
    }
    else
    {
        ...
        /* 如果有上传图片，删除原来的商品图 */
        $sql = "SELECT goods_thumb, goods_img, original_img, goods_front_cover, front_cover_thumb" .
                    " FROM " . $ecs->table('goods') .
                    " WHERE goods_id = '$_REQUEST[goods_id]'";
        $row = $db->getRow($sql);
         if ($goods_front_cover &&$row['goods_front_cover'] && !goods_parse_url($row['goods_front_cover']))
        {
            @unlink(ROOT_PATH . $row['goods_front_cover']);
            @unlink(ROOT_PATH . $row['front_cover_thumb']);
        }
        ...
        /* 如果有上传图片，需要更新数据库 */
         //图片(self)
        if ($goods_front_cover)
        {
            $sql .= "goods_front_cover = '$goods_front_cover', front_cover_thumb = '$front_cover_thumb',";
        }
        if ($goods_img)
        {
    ...
    $goods_front_cover = reformat_image_name('front_cover', $goods_id, $goods_front_cover,  'front_cover');//新的图片
    $goods_thumb = reformat_image_name('goods_thumb', $goods_id, $goods_thumb, 'thumb');
    $front_cover_thumb = reformat_image_name('front_cover_thumb', $goods_id, $front_cover_thumb, 'front_cover_thumb');
    ...
    //(self)
    if ($goods_front_cover !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET goods_front_cover = '$goods_front_cover' WHERE goods_id='$goods_id'");
    }
    //(self)
    if ($front_cover_thumb !== false)
    {
        $db->query("UPDATE " . $ecs->table('goods') . " SET front_cover_thumb = '$front_cover_thumb' WHERE goods_id='$goods_id'");
    }
    if ($original_img !== false)
    {
...
elseif ($_REQUEST['act'] == 'toggle_new')
{
    ...
    $db->query(" UPDATE  " . $ecs->table('goods') . " SET  `is_new` =0 where is_new = 1");
...
elseif ($_REQUEST['act'] == 'toggle_hot')
{
    ...
    $db->query(" UPDATE  " . $ecs->table('goods') . " SET  `is_hot` =0 where is_hot = 1");
