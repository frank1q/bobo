...
&& ($_FILES["Filedata"]["size"] < 200000))
  ...
  if ($_FILES["Filedata"]["error"] > 0)
   ...
   // echo "临时文件: " . $_FILES["Filedata"]["tmp_name"] . "<br />";
    ...
    $imgList = getFile('../model_img/'.$goods_id,'png');
    // var_dump($imgList);
		$attachdir_G = "../model_img/".$goods_id.'/';
    $attachdir_T = $attachdir_G."/thumb_img/";
		if (!file_exists($attachdir_G)) mkdir($attachdir_G);
    if (!file_exists($attachdir_T)) mkdir($attachdir_T);
	  //保存图片
    $ifilename = $_FILES["Filedata"]["name"];
      	//move_uploaded_file($_FILES["Filedata"]["tmp_name"],'./'.$_FILES["Filedata"]["name"]);
		copy($_FILES["Filedata"]["tmp_name"],$attachdir_G.$ifilename);
    
    // var_dump($imgList);
		move_uploaded_file($_FILES["Filedata"]["tmp_name"],$attachdir_G.$ifilename);
    ...
    if(!in_array($ifilename, $imgList)){
        echo '<div class="oneimg"><img src="../'.preg_replace("/..\//","",$attachdir_T.$ifilename,1).'" alt=""><p>'.$_FILES["Filedata"]["name"].'</p></div> ';
    }
    // echo "<input type=\"text\" name=\"img_urlA[]\"  value=\"".preg_replace("/..\//","",$attachdir_G.$ifilename,1);
      }
    }
  }
else
...
class ImageCrop{   
    ...
    function Crop($dst_width,$dst_height,$mode=1,$dst_file=''){   
        ...
        $bg = imagecolorallocatealpha($this->dImage,0,0,0,0);   
